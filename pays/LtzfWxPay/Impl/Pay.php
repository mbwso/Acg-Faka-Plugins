...
namespace App\Pay\LtzfWxPay\Impl;
...
class Pay extends Base implements \App\Pay\Pay
{
    ...
    public function trade(): PayEntity
    {
        ...
        if (!Signature::isMobile()) {
            //扫码支付
            $url = $this->config['url'] . '/api/wxpay/native';
        }else{
            ...
            if(!empty($_SERVER['HTTP_USER_AGENT']) && stripos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false){
                //公众号支付
                $url = $this->config['url'] . '/api/wxpay/jsapi_convenient';
            }else{
                //H5支付
                $url = $this->config['url'] . '/api/wxpay/jump_h5';
            }
        ...
        if (!Signature::isMobile()) {
            ...
            $payEntity->setUrl($json['data']['QRcode_url']);
        } else {
