...
插件安装完成后，我们`启动插件`，然后打开`商品管理`，找到我们要配置的商品，然后拉到最下面，我们可以看到多了很多选项，这里配置的信息作为商品的信息，方便我们实现想要实现的功能。

![](Images/set.png)
> 需要注意的是当请求方式为`不发送`时才不会向这个地址发送数据
>
> 重试次数是指通知失败时重新尝试通知的次数
>
> 结果标识符 就是返回数据里用于标志是否成功的字段 
>
> 例如返回结果为{"code":"1","msg":"success"}
>
> 当code的结果是1才认为成功时，code就是标志符
>
> 此参数仅在返回数据是json格式时有效，留空将会判断返回结果是否包含`成功状态码`
>
> 成功状态码 就是上述返回结果中 code的值为1 就认为成功
>
> 成功失败信息就是通知成功和失败后在订单中显示的信息
